...
public class ItemGenerator : MonoBehaviour
{
    ...
    public void SpawnGoal(Corridor corridor, CorridorGoal prefab, int edge_padding = 1)
    {
        ...
        float length = (corridor.corridorLength / 2f) - offset_from_edge;
        float x = corridor.x;
        float y = corridor.y;
        ...
        switch (corridor.direction)
        {
            case Direction.North:
                ...
                inst.BuildCollisionBox(1 + (edge_padding * 2), corridor.corridorLength);
                break;
            case Direction.East:
                inst = Instantiate(prefab, new Vector3((float)(x + length), y, 0), Quaternion.identity);
                inst.BuildCollisionBox(corridor.corridorLength, 1 + (edge_padding * 2));
                break;
